...
def get_config():
    """Configuration for using the Actor-Critic method with trained cat-likeness critic"""
    config = get_base_config()  # Start with base config
     
    # Set logging and run name
    config.logdir = "logs/ddpo_training"
    config.run_name = "ddpo_cat_unlearn_2"
    
    # Use cat-likeness as reward function (same as critic training)
    ...
    config.prompt_fn = "cats_clip_dataset"
