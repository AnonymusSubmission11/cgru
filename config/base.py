...
def get_config():
    ...
    # whether to normalize advantages (zero mean, unit variance)
    train.normalize_advantages = False
    # time weighting factor for advantages: exp(beta * t/T) where t is timestep and T is total timesteps
    train.time_weighting_beta = 0.0
    ...
    ###### Actor-Critic Method ######
    # when enabled, the model will use a critic to compute advantages instead of simple reward normalization.
    # this implements the AC method described in the theory document.
    config.use_actor_critic = False
    # critic function to use. see `rewards.py` for available critic functions.
    config.critic_fn = "dummy_critic"
    # kwargs to pass to the critic function.
